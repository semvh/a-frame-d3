...
d3.parliament = function() {
    ...
    function parliament(data) {
        data.each(function(d) {
            ...
            var container = svg.select("a-scene");
            if (container.empty()) {
            ...
            var circlesEnter = circles.enter().append("a-box");
            circlesEnter.attr("class", seatClasses);

            var posX = circlesEnter.attr("cx", enter.fromCenter ? 0 : seatX);
            var posZ = circlesEnter.attr("cy", enter.fromCenter ? 0 : seatY);
            circlesEnter.attr("r", enter.smallToBig ? 0 : seatRadius);
            circlesEnter.attr("color", enter.smallToBig ? 0 : "red");

            circlesEnter.attr("position", enter.smallToBig ? 0 :  "0 2 -10");
